-- ============================================================================
-- VALIDATION LOGS DAILY MATERIALIZED VIEW
-- Aggregated daily validation statistics per project
-- ============================================================================
...
SELECT
  ...
  COUNT(*) AS count,
  COUNT(*) FILTER (WHERE vl.recommendation = 'allow') AS approved,
  COUNT(*) FILTER (WHERE vl.recommendation = 'block') AS rejected,
  COUNT(*) FILTER (WHERE vl.recommendation = 'flag') AS failed,
  -- Additional useful metrics
  AVG(vl.risk_score)::INTEGER AS avg_risk_score,
...
-- Function to query validation logs daily with RLS enforcement
-- ============================================================================

CREATE OR REPLACE FUNCTION public.get_validation_logs_daily(
  p_project_id UUID,
...
-- ============================================================================
-- HELPER FUNCTION FOR ALL PROJECTS (SUPERADMIN ONLY)
-- ============================================================================

CREATE OR REPLACE FUNCTION public.get_all_validation_logs_daily(
  p_start_date DATE DEFAULT NULL,
  p_end_date DATE DEFAULT NULL,
...
-- Users should use get_validation_logs_daily() function which enforces RLS
...
-- The actual cron job should be set up via Supabase Dashboard or separately
-- ============================================================================
...
-- This will be idempotent - it checks if the job already exists
DO $$
DECLARE
...
BEGIN
  ...
  SELECT EXISTS (
    ...
    WHERE jobname = 'refresh_validation_logs_daily'
  ) INTO v_job_exists;

  -- Only create if it doesn't exist
  IF NOT v_job_exists THEN
    PERFORM cron.schedule(
      'refresh_validation_logs_daily',           -- Job name
      '0 */6 * * *',                             -- Every 6 hours at minute 0
      'SELECT public.refresh_validation_logs_daily();'
    );
    RAISE NOTICE 'Created pg_cron job: refresh_validation_logs_daily';
  ELSE
    RAISE NOTICE 'pg_cron job refresh_validation_logs_daily already exists';
  END IF;
END $$;
