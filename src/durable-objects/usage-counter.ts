...
export class UsageCounter extends DurableObject {
	...
	async fetch(request: Request): Promise<Response> {
		...
		if (url.pathname === '/check') return this.checkUsage(request)
		if (url.pathname === '/increment') return this.incrementUsage(request)
		if (url.pathname === '/flush') return this.flushUsage(request)
