import { fetchCachedApiKey, getApiKeyFromRequest, sha256Hex, validateOrigin } from '../utils'
...
export async function handleCors(request: Request, env: Env): Promise<Response> {
	...
	if (!originValidation.valid || !originValidation.origin) {
		return new Response(originValidation.error ?? 'Origin not allowed', { status: 403 })
	}
