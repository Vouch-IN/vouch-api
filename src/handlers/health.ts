...
export async function handleHealth(_request: Request, env: Env): Promise<Response> {
	...
	const settled = await Promise.allSettled(
		entries.map(
			async ([name, def]) => [name, await runCheck(def.fn, def.timeout, def.degradedAt)] as const
		)
	)
	...
	const anyCoreDown = Object.entries(components).some(
		([name, c]) => coreNames.has(name) && c.status === 'down'
	)
	const anyValidationDown = Object.entries(components).some(
		([name, c]) => validationNames.has(name) && c.status === 'down'
	)
	const anyNonCoreDown = Object.entries(components).some(
		([name, c]) => !coreNames.has(name) && c.status === 'down'
	)
