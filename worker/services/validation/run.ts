...
export async function runValidations(
	...
	// Disposable domain check via KV list (best-effort, 400ms timeout)
	if (enabledValidations.disposable) {
		...
		try {
			...
			results.checks.disposable = { error: 'disposable_check_failed', latency: performance.now() - disposableStart, pass: true }
		}
	}

	// MX record check with cache (best-effort, 700ms total)
	if (enabledValidations.mx) {
		const mxStart = performance.now()
		try {
			const hasMX: boolean | null = await checkMXRecords(email, domainPart, env)
			if (hasMX === null) {
	...
	// SMTP verification (best-effort, 500ms total)
	if (enabledValidations.smtp) {
		...
		try {
			...
			results.checks.smtp = { error: 'smtp_check_failed', latency: performance.now() - smtpStart, pass: true }
		}
	}

	// Catch-all domain detection (best-effort, 500ms total)
	if (enabledValidations.catchall) {
		...
		try {
			...
			results.checks.smtp = { error: 'catchall_check_failed', latency: performance.now() - catchAllStart, pass: true }
		}
	}
	// IP reputation check
	if (enabledValidations.ip && ip) {
		...
		try {
			...
			results.checks.smtp = { error: 'ip_check_failed', latency: performance.now() - ipStart, pass: true }
		}
	}

	// Device fingerprint check via Durable Object (best-effort)
	if (enabledValidations.device && fingerprintHash && projectId) {
		...
		try {
			...
			results.checks.device = { error: 'device_check_failed', latency: performance.now() - deviceStart, pass: true }
		}
	}
